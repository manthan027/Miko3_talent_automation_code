"""
Video Talent Test Module
========================
Automates testing of Miko3's Video Talent — plays through a series
...
class VideoTalentTest(BaseTalentTest):
    """
    ...
    def __init__(self, adb, config, **kwargs):
        ...
        super().__init__(
            adb=adb,
            config=config,
            talent_name=talent_cfg.get("display_name", "Video Talent"),
            package_name=talent_cfg.get("package", "com.miko.video"),
        ...
        self.video_keywords = vc_cfg.get(
            "video_text_keywords", ["video", "play", "thumbnail"]
        )
    ...
    def execute(self) -> None:
        ...
        for i in range(1, self.max_videos + 1):
            try:
                ...
                self.take_screenshot(f"video_{i}_before")
                ...
                self.take_screenshot(f"video_{i}_playing")
                ...
                self.take_screenshot(f"video_{i}_after")
                self.videos_played += 1
                self.pass_step(f"Video {i} played successfully")
                ...
                logger.error("Video %d error: %s", i, e)
